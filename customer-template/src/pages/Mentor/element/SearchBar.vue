  ...
  <div class="search-wrapper w-fit rounded py-6">
    <div class="search-bar flex justify-center items-center">
      <div class="search-location relative">
        <button @click="toggleLocationSelector" class="btn-location h-12 flex items-center space-x-2 pl-3 rounded-lg bg-gray-100 w-48 focus:outline-none hover:outline-none">
          <SvgIcon icon="locationIcon" />
          <span class="pl-3 inline-block text-center">{{ chosenLocation || 'Địa điểm' }}</span>
        </button>

        <div v-if="showLocationSelector" class="absolute z-10 top-14 left-0 w-48 bg-white shadow-lg rounded-lg">
          <ul class="p-0">
            <li 
            class="py-2 pl-8 hover:bg-gray-100 cursor-pointer rounded-lg"
            v-for="(location, index) in locations" :key="index" @click="selectLocation(location)">{{ location }}
            </li>
          </ul>
        </div>
      </div>
      <div class="search-input ml-4 relative">
        ...
        <div v-if="showMajorDropdown" class="absolute z-10 top-full left-0 w-[400px] bg-white shadow-lg mt-2 rounded-lg">
          <div class="major-wrapper">
            <h4 class="text-lg font-bold pl-4 py-2 bg-slate-100 rounded-t-lg">Chuyên ngành</h4>
...
export default {
  ...
  setup() {
    ...
    const positions = ref(['Frontend', 'Backend', 'Fullstack', 'Mobile', 'DevOps', 'QA', 'UI/UX', 'Data Science']);
