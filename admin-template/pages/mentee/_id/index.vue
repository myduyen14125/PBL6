<template>
  <div>
    ...
    <div class="content overflow-auto h-90">
      ...
      <ul>
        <li class="list-item px-4 py-2 d-flex min-h-48">
          ...
          <div v-if="person.name" class="content-desc">{{ person.name.trim() }}</div>
        </li>
        <li class="list-item px-4 py-2 d-flex min-h-48">
        ...
        <li class="list-item px-4 py-2 d-flex h-48">
          ...
          <div class="content-desc">{{ getSubjectName(person.subject_id) }}</div>
        </li>
        <li class="list-item px-4 py-2 d-flex h-48">
    ...
    <BaseModal
      v-if="showModal"
      :title="'Confirm'"
      :description=' person.status === "TODO" ? "Would you like to confirm doing status?" : "Would you like to confirm done status?"'
...
export default {
  ...
  created() {
    this.$api.contact.getContactById(this.$route.params.id)
    .then((res) => {
  ...
  methods: {
    ...
    changeStatus(status) {
      if (status === 'TODO') {
        this.$api.contact.markDoingContact(this.$route.params.id)
        .then((res) => {
        ...
        this.$api.contact.markDoneContact(this.$route.params.id)
        .then((res) => {
