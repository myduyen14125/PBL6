    ...
    <div class="content">
      ...
      <ul>
        <li class="list-item px-4 py-2 d-flex min-h-48">
          ...
          <div v-if="service.name" class="content-desc">
            {{ service.name.trim() }}
          </div>
        </li>
        <li class="list-item px-4 py-2 d-flex min-h-48">
...
export default {
  ...
  created() {
    this.$api.service.getServiceById(this.$route.params.id).then((res) => {
  ...
  methods: {
    ...
    changeStatus(status) {
      if (status === "TODO") {
        this.$api.service
          .markDoingService(this.$route.params.id)
          .then((res) => {
        ...
        this.$api.service
          .markDoneService(this.$route.params.id)
          .then((res) => {
