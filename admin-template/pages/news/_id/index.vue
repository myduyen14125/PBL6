  ...
  <div class="h-100 position-relative">
    ...
    <div class="content">
     <div class="d-flex align-items-center mb-2">
      ...
      <h1 v-if="news.title" class="title w-60 break-word white-space ml-4">{{ news.title.trim() }}</h1>
     </div>
     <div class="mb-2">
      <img class="content-image-detail" :src="news.detail_thumbnail_url" alt="">
     </div>
     <ul>
        ...
        <li class="list-item py-2 d-flex align-items-center h-38">
          <div class="color-yellow content-title pl-3 w-90px mr-4">Publish at: </div>
          <div class="content-desc">{{ news.published_at ? formatDateTime(news.published_at) : '-' }}</div>
        </li>
        <li class="list-item py-2 d-flex align-items-center h-57">
          <div class="content-title pl-3 mr-4">Category</div>
          <div class="content-desc" :class="news.category_id ? 'category' : ''">{{ getCategoryName(news.category_id) }}</div>
        </li>
        <li class="list-item py-2 d-flex">
          <div class="content-title pl-3 mr-4">Content</div>
          <div class="content-desc break-word html-content ql-editor" v-html="news.content"></div>
        </li>
        <li class="list-item py-2 d-flex align-items-center h-57">
          <div class="content-title pl-3 mr-4">Published</div>
          <div class="switch-toggle">
            <label class="switch">
              <input v-model="news.published" type="checkbox" @click="markPublic()">
...
export default {
  ...
  methods: {
    ...
    markPublic() {
      this.$api.news.markPublicNews(this.news.id)
        .then((res) => {
