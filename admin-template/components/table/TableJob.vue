          ...
          <tr v-for="(item, index) in data" :key="index" class="table-row">
            <td class="color-primary w-25">
              <span class="td-content">
                <nuxt-link :to="`/job/${item.id}`" class="cursor-pointer color-primary" @click="handleRouting(item.id)">{{ item.title.trim() }}</nuxt-link>
              </span>
            </td>
            <td v-if="item.working_type" class="color-secondary w-15 pt-18px">{{ formatWorkingType(item.working_type) }}</td>
            <td class="color-secondary w-25">
              <span class="td-content">{{ item.introduction.trim() }}</span>
            </td>
            <td class="color-secondary w-10 pt-18px">{{ formatSalary(item.salary) }}</td>
            <td class="color-secondary w-15 pt-18px">{{ formatDateTime(item.deadline) }}</td>
