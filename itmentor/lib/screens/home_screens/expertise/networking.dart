...
class NetworkingScreen extends StatefulWidget {
  ...
  const NetworkingScreen({super.key, required this.networkingId});
...
class _NetworkingScreenState extends State<NetworkingScreen> {
  ...
  Future<List<dynamic>> fetchNetworkingMentors() async {
    try {
      final apiUrl = Uri.https(
          Constants.uri, '/mentor/search', {'expertise': widget.networkingId});
  ...
  Future<void> loadMoreMentors() async {
    ...
    final uri = Uri.https(Constants.uri, '/mentor/search',
        {'expertise': widget.networkingId, 'page': '$currentPage'});
