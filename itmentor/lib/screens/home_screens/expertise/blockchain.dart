...
class BlockchainMentors extends StatefulWidget {
  final String blockchainId;
  const BlockchainMentors({super.key, required this.blockchainId});

  @override
  State<BlockchainMentors> createState() => _BlockchainMentorsState();
}

class _BlockchainMentorsState extends State<BlockchainMentors> {
  ...
  Future<List<dynamic>> fetchNetworkingMentors() async {
    try {
      final apiUrl = Uri.https(
          Constants.uri, '/mentor/search', {'expertise': widget.blockchainId});
  ...
  Future<void> loadMoreMentors() async {
    ...
    final uri = Uri.https(Constants.uri, '/mentor/search',
        {'expertise': widget.blockchainId, 'page': '$currentPage'});
